...
void SendMotion(FbxTime start, FbxTime end, int childCount, FbxNode* rootNode, SOCKET MotionSocket, char* RecvBuf)
{
	int Reset = 0;
					...
					if(k == 1 || k == 2 || k == 3 || k == 4 || k == 5 || k == 6 || k == 7 || k == 8 || k == 26 || k == 27 || k == 65 || k == 66 || k == 67 || k == 68 || k == 88 || k == 89 || k == 91 || k == 94 || k == 95 || k == 97 || k == 99)
					{
						...
						if (k == 1)
						{
							...
							for (int i = 0; i < 3; i++)
							{
								...
								SendMotionBuf[40 + i * 4] = Bytes[0];
								SendMotionBuf[41 + i * 4] = Bytes[1];
								SendMotionBuf[42 + i * 4] = Bytes[2];
								SendMotionBuf[43 + i * 4] = Bytes[3];
...
void SendFacial(FbxTime start, FbxTime end, int childCount, FbxNode* rootNode, FbxAnimStack* animStack, SOCKET FacialSocket, char* RecvBuf)
{
	int Reset = 0;
...
int main()
{
	...
	const char* fbxFilePath = "C:/FBXtest/dance_NewAnimation3.fbx";
	...
	thread motionThread(SendMotion, start, end, childCount, rootNode, MotionSocket, RecvBuf);
	thread facialThread(SendFacial, start, end, childCount, rootNode, animStack, FacialSocket, RecvBuf);
